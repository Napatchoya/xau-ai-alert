...
def send_multiple_patterns_message(all_patterns, shared_df):
    ...
    try:
        ...
        # กรอง patterns ตาม method
        chart_patterns = [p for p in all_patterns if p.get('method', '').startswith('CHART') or 
                         p.get('method', '') == 'RULE_BASED' or
                         p['pattern_name'] in ['DESCENDING_TRIANGLE', 'WEDGE_FALLING', 'DOUBLE_TOP', 
                                               'HEAD_SHOULDERS', 'BULL_FLAG', 'BEAR_FLAG', 'ASCENDING_TRIANGLE']]
        
        candlestick_patterns = [p for p in all_patterns if p.get('method', '').startswith('SINGLE') or 
                               p.get('method', '').startswith('TWO') or p.get('method', '').startswith('THREE') or
                               p['pattern_name'] in ['MARUBOZU', 'HAMMER', 'SHOOTING_STAR', 'DOJI', 
                                                     'ENGULFING_BULLISH', 'ENGULFING_BEARISH', 'MORNING_STAR', 'EVENING_STAR']]
        
        # 1) ข้อความแรก - รวมทุก patterns ที่พบ
        if len(all_patterns) > 1:
            message_1 = f"""🔍 MULTIPLE PATTERNS DETECTED!

⏰ {current_time} | 💰 XAU/USD (1H)

📊 พบแพทเทิร์นหลายตัว (จากสูงไปต่ำ):"""

            # แสดง Chart Patterns ก่อน
            if chart_patterns:
                message_1 += f"""

🔧 Method: CHART_PATTERN"""
                
                chart_count = 1
                for pattern in chart_patterns[:3]:  # แสดงสูงสุด 3 patterns
                    confidence_emoji = "🔥" if pattern['confidence'] > 0.8 else "⭐" if pattern['confidence'] > 0.75 else "✨"
                    # กำหนด action ตาม pattern
                    bearish_patterns = ['DESCENDING_TRIANGLE', 'DOUBLE_TOP', 'HEAD_SHOULDERS', 'BEAR_FLAG', 'WEDGE_RISING']
                    action = "🔴 SELL" if pattern['pattern_name'] in bearish_patterns else "🟢 BUY"
                    
                    message_1 += f"""

{chart_count}. {confidence_emoji} {pattern['pattern_name'].replace('_', ' ')} 🎯BOTทำนาย: {action}
   🎯 Confidence: {pattern['confidence']*100:.1f}%"""
                    chart_count += 1
            
            # แสดง Candlestick Patterns
            if candlestick_patterns:
                message_1 += f"""

🔧 Method: SINGLE_CANDLESTICK"""
                
                candlestick_count = 1
                for pattern in candlestick_patterns[:2]:  # แสดงสูงสุด 2 patterns
                    confidence_emoji = "🔥" if pattern['confidence'] > 0.8 else "⭐" if pattern['confidence'] > 0.75 else "✨"
                    # กำหนด action ตาม pattern
                    bearish_candlestick = ['SHOOTING_STAR', 'EVENING_STAR', 'ENGULFING_BEARISH', 'HANGING_MAN', 'THREE_BLACK_CROWS']
                    action = "🔴 SELL" if pattern['pattern_name'] in bearish_candlestick else "🟢 BUY"
                    
                    message_1 += f"""

{candlestick_count}. {confidence_emoji} {pattern['pattern_name'].replace('_', ' ')}. 🎯BOTทำนาย: {action}
   🎯 Confidence: {pattern['confidence']*100:.1f}%"""
                    candlestick_count += 1
        ...
        # 2) ข้อความที่สอง - Original BOT (แก้ไข format)
...
💡 คำแนะนำ: เมื่อมีหลายแพทเทิร์น ควรรอให้ตลาดชัดเจนกว่านี้"""
        ...
        # 3) ข้อความที่สาม - Main Pattern กับรูป
        main_pattern = all_patterns[0]
        
        # สร้างรูปสำหรับ main pattern
...
🎯 ส่งทฤษฎีและรูปประกอบแล้ว {min(valid_pattern_count, 4)} patterns
