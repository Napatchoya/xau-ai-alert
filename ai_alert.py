...
def create_candlestick_chart(df, trading_signals, pattern_info):
    ...
    try:
        ...
        if pattern_name == 'DOUBLE_TOP':
            draw_double_top_on_chart(ax1, chart_df)
        elif pattern_name == 'HEAD_SHOULDERS':
            draw_head_shoulders_on_chart(ax1, chart_df)
        elif pattern_name == 'DOUBLE_BOTTOM':
            draw_double_bottom_on_chart(ax1, chart_df)
        elif pattern_name == 'ASCENDING_TRIANGLE':
            draw_ASCENDING_TRIANGLE_on_chart(ax1, chart_df)
        elif pattern_name == 'BULL_FLAG':
            draw_BULL_FLAG_on_chart(ax1, chart_df)
        elif pattern_name == 'DESCENDING_TRIANGLE':
            draw_DESCENDING_TRIANGLE_on_chart(ax1, chart_df)
        elif pattern_name == 'SYMMETRICAL_TRIANGLE':
            draw_SYMMETRICAL_TRIANGLE_on_chart(ax1, chart_df)    
        elif pattern_name == 'BEAR_FLAG':
            draw_BEAR_FLAG_on_chart(ax1, chart_df)
        elif pattern_name == 'PENNANT':
            draw_PENNANT_on_chart(ax1, chart_df) 
        elif pattern_name == 'WEDGE_RISING':
            draw_WEDGE_RISING_on_chart(ax1, chart_df)
        elif pattern_name == 'WEDGE_FALLING':
            draw_WEDGE_FALLING_on_chart(ax1, chart_df)
        elif pattern_name == 'CUP_AND_HANDLE':
            draw_CUP_AND_HANDLE_on_chart(ax1, chart_df)
        elif pattern_name == 'INVERSE_HEAD_SHOULDERS':
            draw_INVERSE_HEAD_SHOULDERS_on_chart(ax1, chart_df)
        elif pattern_name == 'RECTANGLE':
            draw_RECTANGLE_on_chart(ax1, chart_df)
        elif pattern_name == 'DIAMOND':
            draw_DIAMOND_on_chart(ax1, chart_df)  
        # ✅ Harmonic Patterns
        elif pattern_name in ['GARTLEY', 'BUTTERFLY', 'BAT', 'CRAB']:
            if 'points' in pattern_info and pattern_info['points']:
                draw_harmonic_on_chart(ax1, chart_df, pattern_info['points'], pattern_name)
        
        elif pattern_name == 'AB_CD':
            if 'points' in pattern_info and pattern_info['points']:
                draw_abcd_on_chart(ax1, chart_df, pattern_info['points'])
        
        # ✅ Elliott Wave
        elif pattern_name in ['ELLIOTT_WAVE_5', 'ELLIOTT_WAVE_3']:
            if 'wave_points' in pattern_info and pattern_info['wave_points']:
                draw_elliott_wave_on_chart(ax1, chart_df, pattern_info['wave_points'], pattern_name)
...
def draw_wedge_falling_on_chart(ax, df):
    ...
    try:
        ...
        if len(high_points) >= 3 else high_points[-2:]
        recent_lows = low_points[-3:]
        if len(low_points) >= 3 else low_points[-2:]
...
def draw_harmonic_on_chart(ax, df, points, pattern_name):
    """วาด Harmonic Pattern (XABCD) บนกราฟ"""
    try:
        # สีสำหรับแต่ละจุด
        colors = {
            'X': '#ff0000',  # Red
        ...
        if len(valid_points) < 4:
            print(f"⚠️ Not enough valid points for {pattern_name} (found {len(valid_points)})")
            return
        
        # 🎯 วาดจุด XABCD
        for point in valid_points:
            ax.scatter([point['idx']], [point['price']], 
                      color=point['color'], s=250, marker='o', 
            ...
            ax.text(point['idx'], point['price'] + 8, 
                   f"{emoji.get(point['name'], '⭐')} {point['name']}", 
                   ha='center', va='bottom', 
                   color=point['color'], 
...
def create_mock_harmonic_pattern(df, pattern_type):
    """สร้าง mock Harmonic pattern สำหรับทดสอบ"""
    try:
        highs = df['high'].values
        lows = df['low'].values
        
        # หาจุด swing points จริงจากข้อมูล
        swing_highs = []
        swing_lows = []
        ...
        if len(all_swings) >= 5:
            selected = all_swings[-5:]  # เอา 5 จุดล่าสุด
            ...
            # Fallback: สร้างจุดจากข้อมูลที่มี
            last_idx = len(df) - 1
            points = {
                'X': (last_idx - 40, float(df['low'].iloc[-40]), 'low'),
        ...
        print(f"Mock harmonic pattern error: {e}")
        return {
            'pattern_id': 0,
        ...
        if message_sent_this_hour['harmonic'] != current_hour:
            ...
            def send_harmonic_task():
                try:
                    ...
                    # ใช้ฟังก์ชันตรวจจับแพทเทิร์นขั้นสูง
                    pattern_result, telegram_msg = detect_all_patterns_enhanced(
                        shared_df, 'XAUUSD', '1H'
                    )
                    ...
                    print(f"❌ Harmonic send error: {e}")
                    error_msg = f"❌ Harmonic AI Error\n{str(e)[:100]}"
                    send_telegram(error_msg)
            ...
            return jsonify({
                ...
                "mode": "TELEGRAM_SENT",
                "time": current_time,
                "patterns": "GARTLEY, BUTTERFLY, BAT, CRAB, ELLIOTT_WAVE",
                "telegram_sent": True
            })
        else:
            return jsonify({
                ...
                "mode": "KEEP_ALIVE",
                "time": current_time,
                "note": "Harmonic signal already sent this hour"
            })
